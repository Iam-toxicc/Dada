...
async def manage_sessions_handler(c, cb):
    ...
    try:
        ...
        for a in auths.authorizations:
            if a.current:
                # 🔥 FIX: Ab 'ignore' ki jagah isme bhi KILL ka callback daal diya hai
                buttons.append([InlineKeyboardButton(f"🟢 THIS BOT ({a.app_name}) | ❌ KILL", callback_data=f"kill_{order_id}_{a.hash}")])
            else:
